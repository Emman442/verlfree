...
import { Search, Filter, Clock, Star, Check, Sparkles, Users } from "lucide-react";
...
export default function JobBoard() {
  ...
  const { data: firestoreJobs, isLoading: jobsLoading } = useCollection(jobsQuery);

  const displayJobs = useMemo(() => {
    const combined = [...(firestoreJobs || [])];
    // Only add mock jobs if they don't exist in firestore (by ID check)
    MOCK_JOBS.forEach(mock => {
      if (!combined.find(j => j.id === mock.id)) {
        combined.push(mock);
      }
    });
    return combined;
  ...
  const handleApply = () => {
    ...
    // For demo purposes, if it's a real job we write to DB
    if (!selectedJob.isDemo) {
      ...
      <div className="container mx-auto px-4 pt-32 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-3 space-y-4">
            {jobsLoading ? (
              <div className="p-12 text-center text-muted-foreground animate-pulse">Scanning open contracts...</div>
            ) : (
